...
Entry point of the inceptions methodology. Role: `A1-drafter` (main chat).
...
2. Classify the request (Step 0) per `R16-classify`: **[A] Question** (asks) or **[B] Initiative/task** (explicit "do" / research).
3. If unclear — ask clarification (`R9-ask`).
4. If the request relates to an existing inception in `inceptions/` — propose to continue (use `inc-continue` to determine where).
5. **[A] Question** → answer directly (light mode, no `inceptions/` artifacts); optionally launch `A2-researcher`/`A3-explorer` for read-only facts. If the answer reveals a real task, propose Step 1.
...
When research is needed (type A answer; type B research loop), launch a subagent via the task tool. Role choice: `A3-explorer` for decomposition into independent lines, `A2-researcher` for a narrow single-line question (default for type A), inline only for one-off simple ops. Build the prompt per **Subagent run protocol** (`R15-ambient`), then the line from `R12-model-line`.
